...
function render(photos) {
  ...
  photos.forEach(photo => {
    ...
    const view = button('View', () => showPreview(photo)); const download = document.createElement('a'); download.textContent = 'Download'; download.href = `/api/photos/${encodeURIComponent(photo.filename)}/download`; const remove = button('Delete', () => openDeleteConfirmation(photo)); remove.className = 'delete';
...
function openDeleteConfirmation(photo) { closeModal(); pendingDelete = photo; confirmModal.hidden = false; }
function closeDeleteConfirmation() { confirmModal.hidden = true; pendingDelete = null; }
function showPreview(photo) { closeDeleteConfirmation(); document.querySelector('#modal-image').src = photo.url; document.querySelector('#modal-image').alt = photo.originalName || 'Uploaded photo'; document.querySelector('#modal-title').textContent = photo.originalName || photo.filename; document.querySelector('#modal-meta').textContent = `${formatBytes(photo.size)} · ${new Date(photo.uploadedAt).toLocaleString()}`; document.querySelector('#modal-download').href = `/api/photos/${encodeURIComponent(photo.filename)}/download`; modal.hidden = false; }
